...
function NewOrganizationTemplateGallery() {
    const { Option } = Select;
    ...
    const handleOk = async (e) => {
        ...
        const { success, messages } = await organizationService.create(
            organization
        );
        if (!success) {
    ...
    return (
        <div className="border-[1px]  w-48 rounded-md cursor-pointer hover:border-blue-400 flex items-center">
            <div
                className="h-full w-full flex flex-col justify-center items-center p-2"
                onClick={showModal}
            >
                <AddIcon />
            </div>
...
export default NewOrganizationTemplateGallery;
