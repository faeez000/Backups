(function () {
    ...
    var offsetParent = function (element) {
        return Optional.from(element.dom.offsetParent).map(
            SugarElement.fromDom
        );
    };
    var prevSibling = function (element) {
        return Optional.from(element.dom.previousSibling).map(
            SugarElement.fromDom
        );
    };
        ...
        each(list, function (rowData) {
            if (rowData.section === "colgroup") {
                ...
                each(rowData.cells, function (rowCell) {
                    ...
                    var isLocked = hasNonNullableKey(
                        lockedColumns,
                        start.toString()
                    );
                ...
                maxRows++;
                cells.push(
                    rowdata(rowData.element, currentRow_1, rowData.section)
                );
                rowCount++;
            }
        });
    ...
    var moveBy = function (warehouse, cell, row, column) {
        return Warehouse.findItem(warehouse, cell, eq$1).bind(function (
            detail
        ) {
            var startRow =
    ...
    function DomUniverse() {
        ...
        var isNonEditable = function (element) {
            return (
                isElement(element) &&
                get$2(element, "contenteditable") === "false"
            );
        };
            ...
            return lookupTable(start).bind(function (startTable) {
                return lookupTable(finish).bind(function (finishTable) {
                    if (eq$1(startTable, finishTable)) {
                        ...
                        return ancestors$4(start, finish).shared.bind(function (
                            lca
                        ) {
    ...
    var getEdges = function (
        ...
        return descendant$1(container, firstSelectedSelector).bind(function (
            ...
            return descendant$1(container, lastSelectedSelector).bind(function (
                ...
                return sharedOne$1(lookupTable, [first, last]).map(function (
                    table
                ) {
                    return {
                        first: first,
    ...
    var shiftSelection = function (
        ...
        return getLast(boxes, lastSelectedSelector).bind(function (last) {
            return moveBy$1(last, deltaRow, deltaColumn).bind(function (
                finish
            ) {
    ...
    var extract = function (table, selectedSelector) {
        ...
        var selector =
            ...
            selectedSelector +
            ")";
        var unselectedCells = filterFirstLayer(
            replica,
            "th,td",
            function (cell) {
        ...
        var widthDelta = getTableWidthDelta(
            table,
            house,
            tableSize,
            replicaStats
    ...
    var cloneAppropriateAttributes = function (original, clone) {
        each$1(
            transferableAttributes,
            function (validAttributes, attributeName) {
    ...
    var resizeTable = function () {
        var calcFixedDeltas = function (
            ...
            var clampedDelta = clampNegativeDelta(
                sizes,
                index,
                delta,
                minCellSize
    ...
    var fireObjectResizeStart = function (
        editor,
        target,
        width,
        height,
    ...
    var getResizeHandler = function (editor) {
        ...
        editor.on("ObjectResizeStart", function (e) {
            ...
            if (isTable(targetElm)) {
                ...
                each(
                    editor.dom.select(".mce-clonedresizable"),
                    function (clone) {
                        editor.dom.addClass(
                            clone,
                            "mce-" +
                                getColumnResizingBehaviour(editor) +
                                "-columns"
                        );
                    }
                );
                if (!isPixelSizing$1(table) && isPixelsForced(editor)) {
                    ...
                    !isPercentSizing$1(table) &&
                    isPercentagesForced(editor)
                ) {
                    enforcePercentage(editor, table);
                }
                if (
                    isNoneSizing$1(table) &&
    ...
    var generateSection = function (table, sectionName) {
        var section = child$2(table, sectionName).getOrThunk(function () {
            var newSection = SugarElement.fromTag(
                sectionName,
                owner(table).dom
            );
    ...
    var run = function (
        ...
        return function (
            ...
            return output.bind(function (out) {
                ...
                var resizing =
                    Optional.from(resizeBehaviour).getOrThunk(preserveTable);
    ...
    var uniqueColumns = function (details) {
        ...
        return foldl(details, uniqueCheck, []).sort(function (
            detailA,
            detailB
        ) {
            ...
            var getScopeProperty = function () {
                ...
                if (stringAttributes.length === 0) {
                    ...
                    var isMixed = exists(
                        ...
                        function (attribute) {
                            return (
                                attribute !== baseScope_1 &&
                                contains(scopes_1, attribute)
                            );
                        }
                    );
                    return isMixed
                        ? Optional.none()
                        : Optional.from(baseScope_1);
                }
            };
    ...
    var uniqueRows = function (details) {
        ...
        return foldl(details, rowCompilation, []).sort(function (
            detailA,
            detailB
        ) {
    ...
    var pasteColumnsExtractor = function (before) {
        return function (warehouse, target) {
            return onPasteByEditor(warehouse, target).filter(function (
                details
            ) {
                var checkLocked = before
                    ? firstColumnIsLocked
        ...
        if (newSectionName !== oldSectionName) {
            ...
            if (!sectionElm_1) {
                ...
                if (newSectionName === "thead") {
                    last(
                        children$2(
                            SugarElement.fromDom(tableElm),
                            "caption,colgroup"
                        )
                    ).fold(
                        function () {
                            return tableElm.insertBefore(
                                sectionElm_1,
                                firstTableChild_1
                            );
                        },
        ...
        return lift2(cellOpt, rowsOpt, function (cell, rows) {
            return filter(rows, function (row) {
                return exists(fromDom$1(row.dom.cells), function (rowCell) {
                    return (
                        get$2(rowCell, selector) === "1" || eq$1(rowCell, cell)
                    );
                });
            });
        }).getOr([]);
    ...
    var generateColGroup = function (house, minColRange, maxColRange) {
        if (Warehouse.hasColumns(house)) {
            var colsToCopy = filter(
                Warehouse.justColumns(house),
                function (col) {
                    return (
                        col.column >= minColRange && col.column < maxColRange
                    );
                }
            );
    ...
    var copyCols = function (table, target) {
        ...
        return details.map(function (selectedCells) {
            ...
            var maxColRange =
                lastSelectedCell.column + lastSelectedCell.colspan;
            var fakeColGroups = generateColGroup(
                house,
                minColRange,
                maxColRange
            );
    ...
    var extractDataFromSettings = function (editor, hasAdvTableTab) {
        ...
        var extractAdvancedStyleData = function (dom) {
            return {
                ...
                bordercolor: rgbToHex(dom)(
                    get$1(style, "border-color").getOr("")
                ),
        ...
        var advStyle = hasAdvTableTab
            ? extractAdvancedStyleData(editor.dom)
            : {};
        ...
        return __assign(
            {
                ...
                variable: findVariableFrom(elm.innerHTML),
            },
    ...
    var open = function (editor, selections) {
        var cells = getCellsFromSelection(
            getSelectionStart(editor),
            selections
        );
        if (cells.length === 0) {
    ...
    var applyRowData = function (editor, rows, oldData, data) {
        ...
        if (size(modifiedData) > 0) {
            ...
            var styleModified_1 = typeModified_1
                ? size(modifiedData) > 1
                : true;
    ...
    var applyDataToElement = function (editor, tableElm, data) {
        ...
        const tbody = tableElm.children[1];

        tbody.innerHTML = insertOrReplaceVariable(
            tbody.innerHTML,
            data["variable"]
        );
    ...
    var onSubmitTableForm = function (editor, tableElm, oldData, api) {
        ...
        editor.undoManager.transact(function () {
            ...
            if (size(modifiedData) > 0) {
                ...
                var styleModified = captionModified
                    ? size(modifiedData) > 1
                    : true;
    ...
    var verify = function (
        ...
        return closest$1(after, "td,th", isRoot)
            .bind(function (afterCell) {
                return closest$1(before, "td,th", isRoot).map(function (
                    beforeCell
                ) {
    ...
    var simulate = function (bridge, isRoot, direction, initial, anchor) {
        return closest$1(initial, "td,th", isRoot).bind(function (start) {
            return closest$1(start, "table", isRoot).bind(function (table) {
                ...
                return handle$2(bridge, isRoot, direction).bind(function (
                    range
                ) {
    ...
    var getOtherCells = function (table, target, generators) {
        ...
        return details.map(function (selectedCells) {
            var grid = toGrid(warehouse, generators, false);
            var upOrLeftCells = getUpOrLeftCells(
                grid,
                selectedCells,
                generators
            );
    ...
    var getSelectionTargets = function (editor, selections) {
        ...
        var findTargets = function () {
            return getSelectionStartCellOrCaption(
                getSelectionStart(editor)
            ).bind(function (cellOrCaption) {
        ...
        var onSetupPasteableColumn = function (
            getClipboardData,
            lockedDisable
        ) {
            return function (api) {
    ...
    var addButtons = function (editor, selectionTargets, clipboard) {
        editor.ui.registry.addMenuButton("custom-table", {
            tooltip: "Table",
            icon: "table",
    ...
    function Plugin$1() {
        global.add("custom-table", Plugin);
    }
    function insertOrReplaceVariable(content = "", variable = "") {
        const commentVariablePattern = new RegExp("<!--(.*?)-->", "gm");

        if (commentVariablePattern.test(content)) {
            if (variable.length > 0) {
                return content.replace(
                    commentVariablePattern,
                    `<!--${variable}-->`
                );
            }
            return content.replace(commentVariablePattern, "");
        }
        return (content += `<!--${variable}-->`);
    }
    function findVariableFrom(content) {
        const matchedData = content.match(
